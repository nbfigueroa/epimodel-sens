...
def computeStats(X):
    '''
        Compute mean, median and confidence intervals based on quantiles
    '''
...
def plot_tc_and_peak_infection(I_samples, **kwargs):
    fig, (ax0, ax1) = plt.subplots(1, 2, constrained_layout = True)
    ax0.set_title(r'Distribution of $t_c$', fontsize = 20)
    tc_samples = np.argmax(I_samples, axis = 1)
    ax0.hist(tc_samples, 30, density = True)
    ax0.set_xlabel(r'$t_c$ (days)', fontsize = 15)
    
    ax1.set_title(r'Distribution of $I_{max}$', fontsize = 20)
    I_max_samples = np.max(I_samples, axis = 1)
    I_max_samples = I_max_samples/kwargs['N']
    ax1.hist(I_max_samples, 30, density = True)
    ax1.set_xlabel(r'$I_{max}$ (Proportion of Population)')
...
def mc_SIR_sim_stoch(text_error, rollouts, *prob_params, **kwargs):
    ...
    plotSIR_evolutionStochastic2(S_samples, I_samples, R_samples, Plotoptions, text_error, **kwargs)
    
    plot_tc_and_peak_infection(I_samples, **kwargs)
...
def main():    
    ...
    basefilename           = basefilename+'_quantiles'
