...
def run_MC_stochastic_est_SIR(rollouts, *prob_params, **sim_kwargs):
    ...
    SIR_traces, SIR_params, outputs = model.project(days = sim_kwargs['days'], samples = rollouts, progbar = True, CM_function = model.ContactMatricSoftLockdown())
...
def run(prob_type = 'gamma'):
    ...
    '''
    sim_num             = 5
    sim_kwargs          = loadSimulationParams(sim_num, 0, plot_data = 0, header = 'Compartment-SIR-Soft-Lockdown')
    sim_kwargs['days']  = 70
