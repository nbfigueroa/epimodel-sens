...
def computeSEIR_MCresults(SEIR_traces, SEIR_params, *prob_params, **sim_kwargs):
    ...
    print('Sigma^-1 interval: [', sigma_inv_samples[np.argmin(sigma_inv_samples)], ',', sigma_inv_samples[np.argmax(sigma_inv_samples)],']')
    ...
    if plot_traces:
        ...
        # Fix this one if we want to add the Exposed curve!
