...
def plotIT_realizations(I_samples, R_samples, **kwargs):
    ...
    ax0.set_title(r"Realizations of $I(t)$", fontsize=20)
    ...
    ax01.set_title(r"Realizations of $T(t)$", fontsize=20)
...
def plotSIR_evolutionStochastic(S_variables, I_variables, R_variables, T_variables, Plotoptions, **kwargs):    
    ...
    if show_T:
        ...
        txt1 = r"{tend}: {per:2.3f} {number_scaling} total cases"
        T_end = r"$T_{\rm end}$"
        ax1.text(0.55*days, (total_cases/N) + y_axis_offset, txt1.format(tend=T_end, per=total_cases/scale,number_scaling=number_scaling), fontsize=20, color='m')    
        ...
        if Tf == 90:
            if scenario == 1:
                ax1.text(0.2*Tf, 0.0012, text_error, fontsize=18, bbox=dict(facecolor='red', alpha=0.1))
            else:
                ax1.text(0.5*Tf, 0.0000007, text_error, fontsize=18, bbox=dict(facecolor='red', alpha=0.1))
    ...
    legend = ax1.legend(fontsize=18, loc='upper left')
    legend.get_frame().set_alpha(0.5)
